...
function deserializeUser(user_obj) {
	return new Promise((resolve, reject) => {
		...
		connection.query("SELECT * FROM spark_user WHERE camper_id = ?;", [user_obj.camper_id], (err, user) => {
			...
			deserialized = user[0];
			connection.query("SELECT market.id as market_id, name, icon, staffer, camp_name, city_id FROM market_membership LEFT JOIN market ON market_membership.market_id = market.id WHERE camper_id = ? AND market.disabled = 0 ORDER BY staffer ASC;", [user_obj.camper_id], (err, markets) => {
